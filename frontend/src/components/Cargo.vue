<template>
        ...
        >
            <template #default="{ node, data }">
                <span class="custom-tree-node">
                    ...
                    <span>
                        <el-button size="small" @click="dialogFormVisible_3=true;" :disabled="cargoID!=data.id">修改</el-button>
                        <el-button type="primary" size="small" :icon="Plus" :disabled="cargoID!=data.id" @click="dialogFormVisible_2=true;" circle></el-button>
                        <el-button type="danger" size="small" :icon="Minus" :disabled="cargoID!=data.id" @click="del()" circle></el-button>
                    </span>
                </span>
            </template>
        </el-tree>
    ...
    <el-dialog v-model="dialogFormVisible_3" title="更改节点" width="500">
        <el-form :model="form" label-width="auto" style="max-width: 600px">
        ...
        <div style="text-align: center;">
            <el-button type="primary" v-loading="loading_2" @click="put">提交</el-button>
        </div>
    </el-dialog>
</template>
...
<script lang="ts" setup>
    ...
    const addChild=()=>{
        axios.post(`/cargo/${cargoID.value}`, 
        { "cargoName":form.name,"cargoDescription": form.description})
        .then(function (res) {
            if(res.data.msg=='success'){
                ElNotification({
                    title: 'Success',
    ...
    const del=()=>{
        axios.delete(`/cargo/${cargoID.value}`)
        .then(function (res) {
            if(res.data.msg=='success'){
                ElNotification({
                    title: 'Success',
    ...
    const put=()=>{
        axios.put(`/cargo/${cargoID.value}`,{cargoName:form.name,cargoDescription:form.description})
        .then(function (res) {
            if(res.data.msg=='success'){
                ElNotification({
                    title: 'Success',
